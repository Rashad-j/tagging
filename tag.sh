...
# Check if the latest commit is tagged
    ...
    if [ "$(echo "$latest_tag" | awk -F- '{print NF}')" -eq "2" ]; then
    ...
    if [ "$(echo "$latest_tag" | awk -F- '{print NF}')" -eq "1" ]; then
        ...
        dev=$(echo "$latest_tag" | awk -F- '{print $2}')
        # Extract the numeric part of the dev
        dev_number=${dev##*[^0-9]}
        # Increase the number by 1
        new_dev_number=$((dev_number + 1))
        # Get the new tag
        major_minor_patch=$(echo "$latest_tag" | awk -F- '{print $1}')
